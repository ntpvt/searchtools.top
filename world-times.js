...
(function() {
	...
	var widgetUrl   = 'http://www.worldtimebuddy.com/clock_frame?h=5368361|Q2FsaWZvcm5pYQ==&wt=c1&ap=[TF]';
	var clockWidget = 'c1'.match(/c\d/) != null;	
	...
	function handleMessage(e, method) {
		...
		if (window.frames['frame_' + widgetObjName] == e.source || method == 'hash') {
			...
			if (window[callback] != undefined) {
				var data_obj = wtb_widget_1472049703.ev('('+e.data+')'); 
	...
	// Write CSS to control widget color
		document.write('<style>#wtb-ew-v1 #' + widgetObjName + ',.wtb-ew-v1 #' + widgetObjName + '{ background-color: #e84c3d} #' + widgetObjName + ' .wtb-ew-bottom .wtb-ew-selected{ color: #e84c3d }</style>');
	...
	// Create listener for to receive XDM messages (host must be without the trailing slash). Only for the schedulign widget
		receiveMessage(handleMessage, 'http://www.searchtools.top');
	

	// Create the outside frame	
	// var $outside = $(
	document.write(  '<span class="wtb-ew-outside" id="' + widgetObjName +'">'
					+'	<iframe frameborder="0" name="frame_' + widgetObjName + '" id="frame_' + widgetObjName + '" scrolling="no" class="wtb-ew-locations" style="width: 180px; height: 92px;" src=""></iframe>'
+'	<span class="wtb-ew-bottom" style="width: 182px;">'
+'		<span id="'+ widgetObjName+'_wl"><span class="wtb-ew-powered">Phương Nguyễn<br />Hello everyone</span></span>'
+''
+'		<span title="Switch to 24-hour format" class="wtb-tf wtb-tf-24" format="24" onmousedown="'+ widgetObjName+'.sf(0)">24</span>'
+'		<span title="Switch to AM/PM format"   class="wtb-tf" 			format="AM" onmousedown="'+ widgetObjName+'.sf(1)">AP</span>'
+'		<span style="clear:both;display:block"></span>'
	...
	function initWidget() {
		...
		if (t.length == 0) {
			...
			if (link == null || link.tagName.toLowerCase() != 'a' || link.href.toLowerCase().indexOf('http://www.searchtools.top/') < 0) {
